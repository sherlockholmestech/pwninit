...
fn fetch_ld_impl(ver: &LibcVersion, out_name: impl AsRef<std::path::Path>) -> Result {
...
pub fn fetch_ld(ver: &LibcVersion) -> Result {
    ...
    fetch_ld_impl(ver, format!("ld-{}.so", ver.string_short))
}
...
pub fn fetch_ld_canonical(ver: &LibcVersion) -> Result {
    println!("{}", "fetching linker".green().bold());
    fetch_ld_impl(ver, canonical_ld_name(ver.arch.clone()))
}
